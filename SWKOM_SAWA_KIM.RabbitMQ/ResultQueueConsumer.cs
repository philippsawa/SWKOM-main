...
{
    public class ResultQueueConsumer : RabbitMQService, IQueueConsumer
    {
        private EventingBasicConsumer _consumer;
        private readonly ILogger<ResultQueueConsumer> _logger;

        public ResultQueueConsumer(IOptions<QueueOptions> options, ILogger<ResultQueueConsumer> logger) : base(options.Value.ConnectionString, options.Value.ResultQueueName, logger)
        {
            _consumer = new EventingBasicConsumer(_channel);
            _logger = logger;

            _logger.LogInformation($"ResultQueueConsumer: {QueueName} is created.");
        }
        ...
        {
            _logger.LogInformation($"ResultQueueConsumer: Start receiving from {QueueName} queue.");
            ...
            _consumer.Received += (ch, ea) =>
            {
                try
                {
                    ...
                    _logger.LogInformation($"ResultQueueConsumer: Processed message from {QueueName} queue.");
                }
                catch (Exception ex)
                {
